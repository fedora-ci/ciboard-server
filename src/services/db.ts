...
class DBCollection<T> {
  ...
  async findOrCreate<T>(query: any): Promise<T> {
    ...
    try {
      result = await this.collection.findOneAndUpdate(
        ...
        /** update */
        {
          $setOnInsert: { ...query, _version: 1, _updated: updated },
        },
        /** options */
        {
       ...
       */
      this.fail('findOrCreate() failed for query %o', query);
      throw err;
    }
  ...
  async init(): Promise<void> {
    try {
      ...
      this.collection = this.db.collection<T>(this.collection_name);
